...
func TestRemoteDBM(t *testing.T) {
	...
	actual, status := dbm.CompareExchangeAdvancedStr("xyz", nil, "123", 0, false)
	CheckEq(t, StatusSuccess, status)
	CheckTrue(t, IsNilString(actual))
	actual, status = dbm.CompareExchangeAdvancedStr("xyz", AnyString, AnyString, 1, true)
	CheckEq(t, StatusSuccess, status)
	CheckEq(t, "123", actual)
	actual, status = dbm.CompareExchangeAdvancedStr("xyz", AnyString, NilString, 1, true)
	CheckEq(t, StatusSuccess, status)
	CheckEq(t, "123", actual)
...
func TestIterator(t *testing.T) {
	...
	if status.IsOK() {
		...
		CheckEq(t, "\x00\x00\x00\x00\x00\x00\x00\x00", key)
		CheckEq(t, "one", value)
		strKey, strValue, status = dbm.PopFirstStr(1)
		CheckEq(t, StatusSuccess, status)
		CheckEq(t, "\x00\x00\x00\x00\x00\x00\x00\x01", strKey)
		CheckEq(t, "two", strValue)
